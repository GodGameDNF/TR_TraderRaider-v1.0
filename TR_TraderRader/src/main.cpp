...
ExtraDataList* ExtraDataList_ctor(ExtraDataList* elist)
{
	using func_t = decltype(&ExtraDataList_ctor);
	REL::Relocation<func_t> func{ REL::ID(1329859) };
	return func(elist);
}

void InitItem(ExtraDataList* extra, TESForm* form)
{
	using func_t = decltype(&InitItem);
	REL::Relocation<func_t> func{ REL::ID(639449) };
	return func(extra, form);
}
	...
	void ProcessFactionForms(){
		...
		for (RE::TESFaction* faction : AllFactionForms) {
			if (faction) {
				...
				if (vendorChest) {
					std::stringstream ss;
					ss << std::hex << faction->formID;
					std::stringstream cc;
					cc << std::hex << vendorChest->formID;
...
void CheckVendor(Actor* a)
{
	...
	if (vendorEditorID.find("aravan") != std::string::npos && !(a->IsInFaction(caravanLeader))) {
		...
		//logger::info("정착지 상인");
		TESForm* note = dataHandler->LookupForm(0x80F, "TR_TraderRader.esp");
	...
	if (dChest01) {
		...
		CallGlobalFunctionNoWait(vm, 0, nullptr, new BSFixedString("TR_TraderRader_F4SE"), new BSFixedString("otherChest"), args);
	}
}
...
bool RegisterPapyrusFunctions(RE::BSScript::IVirtualMachine* a_vm)
{
	...
	//a_vm->BindNativeMethod("TR_TraderRader_F4SE"sv, "SendFormID"sv, SendFormID);
